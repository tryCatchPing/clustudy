...
### Isar 버전 정책 (2025-08-18)

- 본 프로젝트는 현재 **Isar v3.x**를 사용합니다.
- 의존성 고정:
  - `isar: 3.1.0+1`
...
- 사유: 리포 전반에서 v3 API(`Isar.open(allSchemas)`, `@collection`, `.where()/.filter()` 빌더 등)를 사용 중이며, v4 마이그레이션 비용 대비 이점이 현재 단계에서는 제한적입니다.
