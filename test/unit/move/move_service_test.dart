...
void main() {
  ...
  group('MoveService.moveNote', () {
    test('moves to another folder (append) and compacts indices', () async {
      final isar = await IsarDb.instance.open();

      final v = await NoteDbService.instance.createVault(name: 'V');
      final f1 = await NoteDbService.instance.createFolder(vaultId: v.id, name: 'F1', sortIndex: 1000);
      final f2 = await NoteDbService.instance.createFolder(vaultId: v.id, name: 'F2', sortIndex: 2000);
    ...
    test('moves before specific note in target folder', () async {
      final isar = await IsarDb.instance.open();
    ...
    test('rejects cross-vault move', () async {
      final v1 = await NoteDbService.instance.createVault(name: 'V1');
      final v2 = await NoteDbService.instance.createVault(name: 'V2');
      final f1 = await NoteDbService.instance.createFolder(vaultId: v1.id, name: 'F1', sortIndex: 1000);
      final f2 = await NoteDbService.instance.createFolder(vaultId: v2.id, name: 'F2', sortIndex: 1000);
      final m = await NoteDbService.instance.createNote(
        vaultId: v1.id,
        folderId: f1.id,
        name: 'M',
  ...
  group('MoveService.moveFolder', () {
    test('reorders folders at vault root level', () async {
      final isar = await IsarDb.instance.open();

      final v = await NoteDbService.instance.createVault(name: 'V');
      final a = await NoteDbService.instance.createFolder(vaultId: v.id, name: 'A', sortIndex: 1000);
      final b = await NoteDbService.instance.createFolder(vaultId: v.id, name: 'B', sortIndex: 2000);
      final c = await NoteDbService.instance.createFolder(vaultId: v.id, name: 'C', sortIndex: 3000);
