...
/// LinkRepository 주입용 Provider. 실제 구현체는 앱 구성 단계에서 override 해야 합니다.
// @riverpod
// LinkRepository linkRepository(LinkRepositoryRef ref) =>
//     throw UnimplementedError('Provide a LinkRepository implementation.');
...
/// 특정 페이지로 들어오는 Backlinks 목록을 스트림으로 제공합니다.
@riverpod
Stream<List<LinkModel>> backlinksToPage(
  Ref ref,
  String pageId,
) {
  final repo = ref.watch(linkRepositoryProvider);
...
@riverpod
Stream<List<LinkModel>> backlinksToNote(
  Ref ref,
  String noteId,
) {
  final repo = ref.watch(linkRepositoryProvider);
